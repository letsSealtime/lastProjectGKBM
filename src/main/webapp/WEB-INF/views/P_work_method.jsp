...
<body>
	<h1>◎ 작업표준서</h1>
	<span>* 모두 기입</span>
	<form method="post" action="p_work_method" class="form"
		enctype="multipart/form-data">
		<div class="form-fields">
			<div class="form-row">
				<label for="작업양식">작업양식<span>*</span></label> <input type="text"
					name="work" value="${select20.work}"> <label for="소모량">소모량<span>*</span></label>
	...
	<!-- 이건 부분조회 -->
	<!-- /searchBySkuCode -->
	<form method="get" action="work_method2">
		<table border="1">
			<thead>
				<tr>
					<th>checkbox</th>
			...
			<tbody>
				<c:forEach var="wmDTO" items="${list}" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="empnos"
							value="${wmDTO.work_method}"></td>
						<td>${loop.count}</td>
						<td>${wmDTO.work_method}</td>
						<td>${wmDTO.sku_code}</td>
						<td>${wmDTO.sku_name}</td>
						<td>${wmDTO.sku_size}</td>
						<td><a href="P_work_method_View_details?type=${wmDTO.work}">${wmDTO.work}</a>
						</td>
						<td>${wmDTO.consumption}</td>
						<td>${wmDTO.create_date}</td>
	...
	<!-- 그래서 안에 아무것도 안넣음 -->
	<form id="deleteForm" method="post" action="p_work_methodDelete"></form>
    ...
    if (searchKeyword == "") {
     ...
     function cancelUpdate() {
         ...
         // 입력란 초기화
         document.querySelector('input[name="work"]').value = "";
         document.querySelector('input[name="consumption"]').value = "";
         document.querySelector('input[name="sku_id"]').value = "";
         document.querySelector('input[name="work_file"]').value = "";
         

         // 숨겨진 work_method 필드 제거
         const hidden = document.querySelector('input[name="work_method"]');
         if (hidden) {
             hidden.remove();
         }
     }
     ...
     function setFormAction(action) {
         ...
         form.action = "p_work_methodudpateList"; // 예: "insert", "update" 같은 URL로 설정
         form.submit();
     }
