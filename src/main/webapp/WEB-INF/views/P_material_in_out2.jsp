...
<body>
	<h1>◎ 원자재출고관리</h1>
	<span>* 모두 기입</span>
	<form method="post" action="p_material_in_out_2" class="form">
...
<span>원자재의 상품코드는 001~036입니다</span>
	...
	<!-- 이건 부분조회 -->
	<!-- /searchBySkuCode -->
	<form method="get" action="material_in_out2_2">
		<table border="1">
			<thead>
				<tr>
			...
			<tbody>
				<c:forEach var="mioDTO2" items="${list}" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="empnos"
							value="${mioDTO2.ib_id}"></td>
						<td>${loop.count}</td>
						<td>${mioDTO2.ib_id}</td>
						<td>${mioDTO2.material_count}</td>
						<td>${mioDTO2.material_price}</td>
						<td>${mioDTO2.sku_code}</td>
						<td>${mioDTO2.sku_name}</td>
						<td>${mioDTO2.sku_size}</td> 
						<td>${mioDTO2.sku_type}</td>
						<td>${mioDTO2.create_date}</td>
						<td>${mioDTO2.modify_date}</td>
						<td>${mioDTO2.remarks}</td>
						<td>${mioDTO2.sku_id}</td>
						<td>${mioDTO2.bill_id}</td>
 ...
 <!-- 여기에 JS로 checkbox 값들이 동적으로 들어감 -->
	...
	<!-- 그래서 안에 아무것도 안넣음 -->
	<form id="deleteForm" method="post" action="p_material_in_outDelete_2"></form>
    ...
    if (searchKeyword == "") {
    	        ...
    	        location.href = "material_in_out2_2"; 
    	    } else {
    	        // sku_code 값으로 조회
    	        location.href = "material_in_out2_2?searchKeyword=" + encodeURIComponent(searchKeyword);
     ...
     function setFormAction(action) {
         ...
         form.action = "p_material_in_outudpateList_2"; // 예: "insert", "update" 같은 URL로 설정
         form.submit();
     }
