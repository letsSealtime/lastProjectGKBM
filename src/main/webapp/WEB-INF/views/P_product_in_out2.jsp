...
<body>
	<h1>◎ 완제품출고관리</h1>
	<span>* 모두 기입</span>
	<form method="post" action="p_product_in_out_2" class="form">
		<div class="form-fields">
			<div class="form-row">
				<label for="완제품수량">완제품수량<span>*</span></label> <input type="text"
					name="product_count" value="${select20.product_count}"> <label
					for="완제품가격">완제품가격<span>*</span></label> <input type="text"
					name="product_price" value="${select20.product_price}">
			</div>
			<div class="form-row">
			...
			<div class="form-row">
				<label for="상품고유번호">상품고유번호<span>*</span></label> <input type="text"
					name="sku_id" value="${select20.sku_id}"> <label for="사원번호">사원번호<span>*</span></label>
				<input type="text" name="empno" value="${select20.empno}">
			</div>
...
<span>완제품의 상품코드는 101~112까지입니다</span>
	...
	<!-- 이건 부분조회 -->
	<!-- /searchBySkuCode -->
	<form method="get" action="product_in_out2_2">
		<table border="1">
			<thead>
				<tr>
			...
			<tbody>
				<c:forEach var="pioDTO" items="${list}" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="empnos"
							value="${pioDTO.ib_id}"></td>
						<td>${loop.count}</td>
						<td>${pioDTO.ib_id}</td>
						<td>${pioDTO.product_count}</td>
						<td>${pioDTO.product_price}</td>
						<td>${pioDTO.sku_code}</td>
						<td>${pioDTO.sku_name}</td>
						<td>${pioDTO.sku_size}</td>
						<td>${pioDTO.sku_type}</td>
						<td>${pioDTO.create_date}</td>
						<td>${pioDTO.modify_date}</td>
						<td>${pioDTO.remarks}</td>
						<td>${pioDTO.sku_id}</td>
						<td>${pioDTO.empno}</td>
	...
	<!-- 그래서 안에 아무것도 안넣음 -->
	<form id="deleteForm" method="post" action="p_product_in_outDelete_2"></form>
	...
	<div class="pagination">
		...
		<c:if test="${currentPage > 1}">
			<form method="get">
				...
				<input type="hidden" name="sku_code" value="${pioDTO2.sku_code}" />
		...
		<c:forEach var="allpages" begin="1" end="${totalPages}">
			<form method="get">
				<input type="hidden" name="currentPage" value="${allpages}" /> <input
					type="hidden" name="sku_code" value="${pioDTO2.sku_code}" />
				<button type="submit"
    ...
    if (searchKeyword == "") {
    	        ...
    	        location.href = "product_in_out2_2"; 
    	    } else {
    	        // sku_code 값으로 조회
    	        location.href = "product_in_out2_2?searchKeyword=" + encodeURIComponent(searchKeyword);
     ...
     function handleEdit() {
         ...
         document.querySelector('input[name="product_count"]').value = cells[3].textContent.trim();
         document.querySelector('input[name="product_price"]').value = cells[4].textContent.trim();
         ...
         document.querySelector('input[name="empno"]').value = cells[13].textContent.trim();
     ...
     function cancelUpdate() {
         ...
         document.querySelector('input[name="empno"]').value = "";
     ...
     function setFormAction(action) {
         ...
         form.action = "p_product_in_outudpateList_2"; // 예: "insert", "update" 같은 URL로 설정
         form.submit();
     }
